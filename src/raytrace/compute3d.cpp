...
int main() {
  ...
  int texture_width = 512;  // WINWIDTH;
  int texture_height = 512; // WINHEIGHT;
  ...
  // filesystem::path cpath = shaderDirPath / "compute.comp";
  ...
  glm::mat4 projMat =
      glm::perspective(glm::radians(60.0f), (float)WINWIDTH / (float)WINHEIGHT,
                       VIEWPORTN, VIEWPORTF);
  // glm::mat4 viewMat = camera.getViewMatrix();
  glm::mat4 viewMat =
      glm::lookAt(camera.pos, glm::vec3(0.0, 0.5, 0.0), glm::vec3(0, 1, 0));
  // frustum rays
  // 0 0 bottom left -1 -1
  ...
  glm::mat4 invVpMat = glm::inverse(vpmat); // from clip space to world space
