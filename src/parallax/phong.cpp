...
void cubeShaderInit_proc2(Shader myShader) {
  ...
  myShader.setFloatUni("depth_scale", 0.1);
  myShader.setVec3Uni("attC", attc);
  myShader.setIntUni("diffuseMap", 0);
  myShader.setIntUni("aoMap", 1);
  myShader.setIntUni("normalMap", 2);
  myShader.setIntUni("heightMap", 3);
  myShader.setIntUni("binary_steps", 10);
  myShader.setIntUni("linear_steps", 20);
}

int main() {
  initializeGLFWMajorMinor(4, 3);
  GLFWwindow *window =
      glfwCreateWindow(WINWIDTH, WINHEIGHT,
                       "Basic Phong With Parallax Relief Mapping", NULL, NULL);
  ...
  // layered-fs
  fs::path diffmapPath = textureDirPath / "layered" /  "layered-cliff-albedo.png";
  fs::path normalMapPath = textureDirPath / "layered" /  "layered-cliff-normal-ogl.png";
  fs::path heightMapPath = textureDirPath / "layered" /  "layered-cliff-height.png";
  ...
  // load shaders
  // cube shader
  std::string vertFileName_t = "parallax.vert";
  std::string fragFileName_t = "parallax.frag";
