...
void main() {
  ...
  vec2 MetallicEmmissive = texture(gMaterial, TexCoords).rg;
  ...
  vec3 albedo = texture(lightBuffer, TexCoords).rgb;

  float spec = texture(gMaterial, TexCoords).w;
  ...
  vec4 coords = RayMarch((vec3(jitt) + reflected * max(minRayStep, -viewPos.z)),
                         hitPos, dDepth);
  ...
  // Get color
  vec3 SSR = textureLod(lightBuffer, coords.xy, 0).rgb *
             clamp(ReflectionMultiplier, 0.0, 0.9) * Fresnel;
