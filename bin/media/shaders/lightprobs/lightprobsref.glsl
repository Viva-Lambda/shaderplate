...
// map to two different
// locations in octahedral space. We shorten the segments slightly to give
// unambigous locations that lead
...
struct LightFieldProbe {
  color sliceColor; // L(p, w) where w is the outgoing direction
  // slice color can be generated with defered rendering and shadow maps
  vec3 closestNormal; // of the neighbouring point at the outgoing w direction
...
GridCoord probeIndexToGridCoord(in LightFieldSurface L, ProbeIndex index) {
  ...
  iPos.y = (index & ((L.probeCounts.x * L.probeCounts.y) - 1)) >>
...
float squaredLength(Vector3 v) { return dot(v, v); }
...
/** Segments a ray into the piecewise-continuous rays or line segments that each
   lie within
    one Euclidean octant, which correspond to piecewise-linear projections in
   octahedral space.
...
TraceResult highResolutionTraceOneRaySegment(
  ...
  float texCoordStep = lightFieldSurface.distanceProbeGrid.invSize.x *
                       (texCoordDistance / maxComponent(abs(texCoordDelta)));
      ...
      if (cosTheta > 0) {
        ...
        float distanceFromProbeToSurface =
            texelFetch(lightFieldSurface.distanceProbeGrid.sampler,
                       ivec3(lightFieldSurface.distanceProbeGrid.size.xy *
                                 startTexCoord,
                             probeIndex),
                       0)
                .r;
  ...
  for (float d = 0.0f; d <= texCoordDistance; d += texCoordStep) {
    ...
    float distanceFromProbeToSurface =
        texelFetch(lightFieldSurface.distanceProbeGrid.sampler,
                   ivec3(lightFieldSurface.distanceProbeGrid.size.xy * texCoord,
                         probeIndex),
                   0)
            .r;

    // Find the corresponding point in probe space. This defines a line through
    // the
    // probe origin
    ...
    if (maxDistFromProbeToRay >= distanceFromProbeToSurface) {
      ...
      vec3 normal = octDecode(
          texelFetch(
              lightFieldSurface.normalProbeGrid.sampler,
              ivec3(lightFieldSurface.distanceProbeGrid.size.xy * texCoord,
                    probeIndex),
              0).xy *
              lightFieldSurface.normalProbeGrid.readMultiplyFirst.xy +
          lightFieldSurface.normalProbeGrid.readAddSecond.xy);
...
bool lowResolutionTraceOneSegment(in LightFieldSurface lightFieldSurface,
  ...
  for (Point2 P = P0; ((P.x * sign(delta.x)) <= end);) {
    ...
    float sceneRadialDistMin =
        texelFetch(lightFieldSurface.lowResolutionDistanceProbeGrid.sampler,
                   int3(hitPixel, probeIndex), 0)
            .r;

    // Distance along each axis to the edge of the low-res texel
    Vector2 intersectionPixelDistance =
        (sign(delta) * 0.5 + 0.5) - sign(delta) * frac(P);
...
// Engine-specific arguments and helper functions have been removed from the
// following code
...
Irradiance3 computePrefilteredIrradiance(Point3 wsPosition) {
  ...
  Vector3 alpha =
      clamp((wsPosition - baseProbePos) / lightFieldSurface.probeStep,
            Vector3(0), Vector3(1));
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    GridCoord probeGridCoord =
        clamp(baseGridCoord + offset, GridCoord(0),
              GridCoord(lightFieldSurface.probeCounts - 1));
    ...
    // Make cosine falloff in tangent plane with respect to the angle from the
    // surface to the probe so that we never
    // test a probe that is *behind* the surface.
    // It doesn't have to be cosine, but that is efficient to compute and we
    // must clip to the tangent plane.
    ...
    float2 temp =
        texture(lightFieldSurface.meanDistProbeGrid.sampler, vec4(-dir, p)).rg;
    float mean = temp.x;
    ...
    Irradiance3 probeIrradiance =
        texture(lightFieldSurface.irradianceProbeGrid.sampler,
                vec4(normalize(irradianceDir), p))
            .rgb;
...
Radiance3 computeGlossyRay(Point3 wsPosition, Vector3 wo, Vector3 n, ...) {
  ...
  int probeIndex;
  if (!trace(lightFieldSurface, worldSpaceRay, hitDistance, hitProbeTexCoord,
             probeIndex, FILL_HOLES == 1)) {
    ...
    return textureLod(lightFieldSurface.radianceProbeGrid.sampler,
                      float3(hitProbeTexCoord, probeIndex), 0)
        .rgb;
  }
}
