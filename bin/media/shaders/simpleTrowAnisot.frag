...
float ndfFn(vec3 normal, vec3 halfwayDir, vec2 roughness);
float lambdaFn(vec3 normal, vec3 halfDir, float alpha);
// traditional Trowbridge Reitz Distribution lambda
float lambdaTFn(vec3 normal, vec3 halfway, vec2 roughness);
// lambda for traditional Trowbridge Reitz Distribution
void lambdaTFnIO(vec3 normal, vec3 halfDir, vec3 viewDir, vec2 roughness,
                 float lambdaArr[2]);
