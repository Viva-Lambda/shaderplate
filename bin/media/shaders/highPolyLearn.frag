...
vec3 getNormalFromMap() {
  vec3 tangentNormal = texture(heightMap1, TexCoords).xyz * 2.0 - 1.0;
...
void main() {
  ...
  vec3 albedo = pow(texture(diffuseMap1, TexCoords).rgb, vec3(2.2));
  // vec3 albedo = texture(diffuseMap1, TexCoords).rgb;
  ...
  float distnce = length(lightPos - FragPos);
  float attenuation = 1.0 / (distnce * distnce);
